...
class Map:
	"""Class to represent a linkage map order and spacing,
	no genotype calls included"""
	def __init__(self, name, map_type, file_path):
		...
		if map_type.lower() == 'rqtl':
			...
			second_line = file.next()
			LGs = second_line.strip().split(",")	
			third_line = file.next()
			...
			print ("Unknown map type, enter one of:[rqtl, mst]\n")
		file.close()
		return None
...
def write_generic_map(filename, Map):
    with open(filename, 'w') as OUTFILE:
        for LG in Map.markers_of_LG.keys():
            for marker in Map.markers_of_LG[LG]:
                cM = Map.cM_of_marker[marker]
                OUTFILE.write("{}\t{}\t{}\n".format(LG, marker, cM ))
...
def write_intersection (outfile, map_1 ,*args):
	...
	with open(outfile, 'w') as OUTFILE: 
           	if not isinstance(map_1, Map):
          		...
          		return None
           	for each_Map in args:
          		if not isinstance(each_Map, Map):
         			print(each_Map + " is not a Map object")
         			return None
           	...
           	for each_Map in args:
          		marker_sets.append(set(each_Map.LG_of_marker.keys()))
         			
           	#Write header line
           	to_write = map_1.name, map_1.name
           	OUTFILE.write("Locus\t{} LG\t{} cM".format (*to_write))
           	for each_Map in args:
          		to_write = each_Map.name, each_Map.name
          		OUTFILE.write("\t{} LG\t{} cM".format (*to_write))
          	OUTFILE.write("\n")
           	...
           	for marker in intersection_marker_set:
          		...
          		for each_Map in args:
         			to_write = each_Map.LG_of_marker[marker], each_Map.cM_of_marker[marker]
         			OUTFILE.write("\t{}\t{}".format(*to_write))
         		OUTFILE.write("\n")
...
def write_union (file, map_1 ,*args):
	...
	with open(file, 'w') as OUTFILE:
           	...
           	#Write header line
           	to_write = map_1.name, map_1.name
           	OUTFILE.write("Marker\t{} LG\t{} cM".format(*to_write))
           	for each_Map in args:
          		to_write = each_Map.name, each_Map.name
          		OUTFILE.write("\t{} LG\t{} cM".format(*to_write))
           	OUTFILE.write("\n")
           	...
           	for marker in union_marker_set:
          		...
          		OUTFILE.write("{0}\t{1}\t{2}".format(*map_1_to_write))
          		for each_Map in args:
         			map_to_write = each_Map.LG_of_marker.get(marker, "NA"), each_Map.cM_of_marker.get(marker, "NA")
         			OUTFILE.write("\t{0}\t{1}".format(*map_to_write) )
          		OUTFILE.write("\n")
...
def align_maps (map_1, map_2):
	...
	if a pair of Lgs share only one marker, an error will be returned
	and no renaming will be done.
	Unmatched LGs in map_2 will be prefixed with 'un_'.
	returns a tuple of dictionaries that describe the matches made.
	"""
	...
	for LG_in_map_1 in map_1.markers_of_LG.keys():
		for LG_in_map_2 in map_2.markers_of_LG.keys():
			...
			if len(overlap) > 1:
				...
				print("MATCH, LGs: {}, {} matched at {} / {} loci, ({})".format(LG_in_map_1, LG_in_map_2, len(overlap), possible, float(len(overlap))/(possible)))
				...
				for x in overlap: print ("NONMATCH, LGs: {}, {} share only one marker: {}".format(LG_in_map_1, LG_in_map_2, x))
...
class Marker_data_dev:
	"""Class to represent the genetic data and Map positions across
	a set of markers, individuals, and linkage groups.
	Input data format is rqtl-like with a cM line.
	Map positions can be extracted from mst output files"""
		...
		with open(file_name, 'r') as INFILE:
			...
			for line in INFILE:
				...
				if line_count == marker_name_line:
					...
					LGs = line.strip().split(",")
					LGs.pop(0)
					self.LGs = LGs
				elif line_count == cM_line:
					cMs = line.strip().split(",")
					cMs.pop(0)
					self.cMs = cMs
				elif line_count >= genotype_start_line:
					ind_genotypes_for_marker = dict()
					geno_data = line.strip()
					# remap genotypes
					geno_data = geno_data.replace(",AA", ',a')
					geno_data = geno_data.replace(",AB", ',b')
					geno_data = geno_data.replace(",BB", ',b')
					geno_data = geno_data.split(",")
					ind = geno_data.pop(0)
					self.inds.append(ind)
					for ii in range(len(self.markers)):
						ind_genotypes_for_marker[self.markers[ii]] = geno_data[ii]
					self.data_for_ind[ind] = ind_genotypes_for_marker

		for ii in range(len(self.markers)):
			self.LG_of_marker[self.markers[ii]] = self.LGs[ii]
			self.cM_of_marker[self.markers[ii]] = self.cMs[ii]
		return None
		
	def write_mst(self, file_name, inc_header = 'yes', p_value = .00001, 
								pre_est = 'yes', detect = 'yes', obj_func = "COUNT"):
		"writes out the marker data in MSTmap format including header"
		with open(file_name, 'w') as mst_file:
			header = ["population_type DH\n",
				...
				"number_of_loci " + str(len(self.markers)) + "\n",
				"number_of_individual " + str(len(self.inds)) + "\n",
				"\n" ]
			mst_file.writelines(header)
			# write ind names
			mst_file.write('locus_name\t')
			mst_file.write('\t'.join(self.inds) + '\n')
			# write one line per marker
			for marker in self.markers:
				mst_file.write(marker)
				for ind in self.inds:
					mst_file.write('\t' + self.data_for_ind[ind][marker])
				mst_file.write('\t\n')
		return None
		
	def write_rqtl(self, file_name):
		with open(file_name, 'w') as OUTFILE:
			OUTFILE.write(self.pop_name + ",")
			OUTFILE.write(",".join(self.markers) + "\n")
			OUTFILE.write(",")
			OUTFILE.write(",".join([self.LG_of_marker[marker] for marker in self.markers]) + "\n")
			OUTFILE.write(",")
			OUTFILE.write(",".join([self.cM_of_marker[marker] for marker in self.markers]) + "\n")
			for ind in self.inds:
				OUTFILE.write(ind + ",")
				OUTFILE.write(",".join([self.data_for_ind[ind][marker] for marker in self.markers]) + "\n")
		return None
		
	def import_mst_map_positions(self, mst_file):
		in_map = Map('dummy_name', 'mst', mst_file)
		for rqtl_marker in self.LG_of_marker.keys():
			if rqtl_marker in in_map.LG_of_marker.keys():
				self.LG_of_marker[rqtl_marker] = str(in_map.LG_of_marker[rqtl_marker])
				self.cM_of_marker[rqtl_marker] = str(in_map.cM_of_marker[rqtl_marker])
			else:
				self.LG_of_marker[rqtl_marker] = '9999'
				self.cM_of_marker[rqtl_marker] = '.99'
		self.markers.sort(key = lambda k: float(self.cM_of_marker[k]))		
		self.markers.sort(key = lambda k: int(self.LG_of_marker[k]))
		return None
